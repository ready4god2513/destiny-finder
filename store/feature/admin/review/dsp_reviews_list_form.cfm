...
<cfif qry_get_reviews.recordcount gt 0>					
	...
	<cfset reviewList = ListAppend(reviewList, Review_ID)>
		...
		<tr>
			...
			<a href="#self#?fuseaction=feature.reviews&do=display&Review_ID=#Review_ID##request.token2#" target="store"><strong>#LEFT(title, 50)#</strong></a><br/>
				
				<strong>re:</strong> <a href="#self#?fuseaction=feature.display&Feature_ID=#Feature_ID##request.token2#" target="store">#LEFT(Feature_Name, 50)#</a><br/>

			<em>by <cfif Anonymous is 1>a member<cfelseif user_ID><a href="#self#?fuseaction=feature.reviews&do=list&uid=#user_ID##request.token2#" target="store"><cfif len(anon_name)>#anon_name#<cfelse>#username#</cfif></a><cfelse>#anon_name#</cfif>
